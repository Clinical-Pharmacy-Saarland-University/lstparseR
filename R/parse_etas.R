...
fetch_etas <- function(lst, rse_digits = NA, shk_digits = NA) {
  ...
  subheader <- "OMEGA - COV MATRIX FOR RANDOM EFFECTS - ETAS"
  rse_block <- .f_get_block_values(lst, header1, header2, subheader)
  
  # Calculate the RSE
  if (!is.na(rse_digits)) {
    omerses <- round(rse_block$values * 100 / df$Value, rse_digits)
  } else {
    omerses <- rse_block$values * 100 / df$Value
  }
